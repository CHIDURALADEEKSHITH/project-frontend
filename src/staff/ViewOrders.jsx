...
function ViewOrders() {
  ...
  const loadOrders = () => {
    api.get("/staff/viewallorders").then((res) => {
      setOrders(res.data);
    });
  };
  ...
  const confirmOrder = async (orderId) => {
    try {
      const res = await api.post("/staff/confirmorder", { orderId });
      alert(res.data);
      loadOrders();
    } catch (err) {
      alert(err.response?.data || "Confirm failed");
    }
  };
  ...
  const cancelOrder = async (orderId) => {
    ...
    try {
      const res = await api.post("/staff/cancelorder", {
  ...
  const setDeliveryDate = async (orderId) => {
    ...
    try {
      const res = await api.post("/staff/setdeliverydate", {
        orderId: String(orderId),
        expectedDate,
      });
      ...
      alert(err.response?.data || "Set delivery date failed");
    }
  };
